...
`PrayerTimesProvider` is an interface. `ExternalPrayerTimesProvider` uses islomapi.uz as the prayer time provider. Daily prayer times are fetched with `GET /api/daily?region=Toshkent&month=4&day=28`. The provider also includes a monthly helper for `GET /api/monthly?region=Toshkent&month=4`, which can be used for monthly prefetching. `PrayerTimesService` caches the result in `prayer_times`. Set `PRAYER_API_BASE_URL=https://islomapi.uz` in Railway. If an older Aladhan URL is accidentally left in the environment, the provider normalizes it back to islomapi.uz.`
