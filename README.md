...
`PrayerTimesProvider` is an interface. `ExternalPrayerTimesProvider` now uses Aladhan Prayer Times API as the primary provider: `GET /v1/timingsByCity/{DD-MM-YYYY}` with `city`, `country` and `timezonestring` parameters. `PrayerTimesService` caches the result in `prayer_times`. Set `PRAYER_API_BASE_URL=https://api.aladhan.com/v1` in Railway. `https://aladhan.com` and `https://api.aladhan.com` are normalized to the API `/v1` base automatically. Optional `PRAYER_API_METHOD` and `PRAYER_API_SCHOOL` can be configured when a specific calculation method is needed.
