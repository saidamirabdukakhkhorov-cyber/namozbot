...
class MissedPrayersRepository(BaseRepository):
    async def create(self, *, user_id: int, prayer_name: str, prayer_date: date, source: str = "manual", daily_prayer_id: int | None = None, qazo_calculation_id: int | None = None):
        stmt = insert(MissedPrayer).values(user_id=user_id, prayer_name=prayer_name, prayer_date=prayer_date, status="active", source=source, daily_prayer_id=daily_prayer_id, qazo_calculation_id=qazo_calculation_id).on_conflict_do_nothing(index_elements=["user_id", "prayer_name", "prayer_date"], index_where=(MissedPrayer.status == "active")).returning(MissedPrayer)
        row = (await self.session.scalars(stmt)).first(); return row, row is not None
    async def summary(self, user_id: int, start_date: date | None = None, end_date: date | None = None, sources=None, qazo_calculation_id: int | None = None):
        ...
        if start_date: stmt = stmt.where(MissedPrayer.prayer_date >= start_date)
    ...
    async def count_by_prayer(self, user_id: int, prayer_name: str, sources=None):
        stmt = select(func.count()).select_from(MissedPrayer).where(MissedPrayer.user_id == user_id, MissedPrayer.prayer_name == prayer_name, MissedPrayer.status == "active")
        if sources: stmt = stmt.where(MissedPrayer.source.in_(list(sources)))
        return int(await self.session.scalar(stmt) or 0)
    async def complete_oldest(self, user_id: int, prayer_name: str, count: int, sources=None):
        if count <= 0:
            raise ValueError("Count must be greater than zero")
        stmt = select(MissedPrayer).where(MissedPrayer.user_id == user_id, MissedPrayer.prayer_name == prayer_name, MissedPrayer.status == "active")
        if sources: stmt = stmt.where(MissedPrayer.source.in_(list(sources)))
    ...
    async def undo_completion_action(self, user_id: int, action_id: int):
        ...
        await self.session.execute(update(MissedPrayer).where(MissedPrayer.id.in_(action.missed_prayer_ids), MissedPrayer.user_id == user_id).values(status="active", completed_at=None, updated_at=now))
