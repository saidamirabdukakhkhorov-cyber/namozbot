...
async def state_text_handler(message: Message, current_user, session, is_admin: bool):
    ...
    if state.state == "waiting_custom_city":
        city = text[:120].strip()
        if not city:
            await message.answer(t(current_user.language_code, "city.custom_prompt"))
            return
        ...
        if source == "settings":
            ...
            await message.answer(
                t(current_user.language_code, "onboarding.done", city=city),
                reply_markup=main_menu_keyboard(current_user.language_code, is_admin),
            )
        return
    ...
    if state.state == "waiting_qazo_custom_date":
        ...
        try:
            ...
            return
        if day > date.today():
            await message.answer("Kelajak sanasini qazo sifatida kiritib bo'lmaydi.")
            return

        _, created = await MissedPrayersRepository(session).create(
    ...
    if state.state == "waiting_qazo_complete_count":
        ...
        if count > max_count:
            await message.answer(
                f"Sizda active {prayer_label(current_user.language_code, prayer)} qazolari: {max_count} ta."
            )
            return
        ...
        await StatesRepository(session).clear(current_user.id)
        await message.answer(
            f"✅ Belgilandi\n\n{count} ta {prayer_label(current_user.language_code, prayer)} ado qilingan deb belgilandi.",
            reply_markup=undo_keyboard(action.id),
        )
        return
    ...
    if state.state.startswith("calc_waiting"):
        ...
        await message.answer(
            "Qaysi namozlarni hisoblaymiz?",
            reply_markup=calculator_prayers_keyboard(current_user.language_code, []),
        )
        return
