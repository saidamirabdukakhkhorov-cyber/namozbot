...
async def today_handler(message: Message, current_user: User, session):
    ...
    for prayer in PRAYER_NAMES:
        prayer_dt = service.combine(dto.prayer_date, dto.as_dict()[prayer], current_user.timezone)
        daily = await repo.upsert_pending(user_id=current_user.id, prayer_name=prayer, prayer_date=dto.prayer_date, prayer_time=prayer_dt)
        lines.append(f"{prayer_label(lang, prayer)}: {dto.as_dict()[prayer].strftime('%H:%M')} — {t(lang, 'status.' + daily.status)}")
